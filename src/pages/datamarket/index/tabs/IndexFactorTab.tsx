...
import { IndexFactor, getIndexFactorList, syncIndexFactor, IndexFactorSyncPayload, IndexBasic, getIndexBasicList } from '@app/api/datamarket/index.api';
...
export const IndexFactorTab: React.FC = () => {
  ...
  const [syncTradeDate, setSyncTradeDate] = useState<AppDate | null>(null);
  ...
  const handleSyncOpen = () => {
    ...
    setSyncTradeDate(syncPayload.trade_date ? dayjs(syncPayload.trade_date) : null);
    setSyncOpen(true);
  };
    ...
    <>
      ...
      <BaseModal
        ...
        onOk={async () => {
          ...
          if (selectedSyncCodes.length === 0 && !syncTradeDate) {
            notificationController.warning({ message: '请至少选择指数代码或指定交易日期' });
            return;
          }
          setSyncLoading(true);
          try {
            const payload: IndexFactorSyncPayload = {
              ...
              trade_date: syncTradeDate ? Dates.format(syncTradeDate, 'YYYY-MM-DD') : undefined,
      ...
      >
        <BaseForm layout="vertical">
          ...
          <BaseForm.Item label="交易日期">
            <DayjsDatePicker
              format="YYYY-MM-DD"
              value={syncTradeDate}
              onChange={(val) => setSyncTradeDate(val)}
              placeholder="选择交易日期（可选，与指数代码至少填一个）"
              style={{ width: '100%' }}
            />
          </BaseForm.Item>
