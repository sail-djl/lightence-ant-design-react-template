...
export const CPITab: React.FC = () => {
  const { query, setQuery, rows, loading, pagination, total, fetchData } = useMacroData<CPI, CPIQuery>({
    fetchFn: async (params) => {
      const res = await getCPIList({
        skip: params.skip,
        limit: params.limit,
        month: params.month,
  ...
  const columns: ColumnsType<CPI> = [
    { title: '月份', dataIndex: 'month', key: 'month', align: 'center', width: 120 },
    { title: '全国当月值', dataIndex: 'nt_val', key: 'nt_val', align: 'right', render: (v) => formatNumber(v, 2) },
    { title: '全国同比（%）', dataIndex: 'nt_yoy', key: 'nt_yoy', align: 'right', render: (v) => formatNumber(v, 2) },
    { title: '全国环比（%）', dataIndex: 'nt_mom', key: 'nt_mom', align: 'right', render: (v) => formatNumber(v, 2) },
    { title: '城市当月值', dataIndex: 'town_val', key: 'town_val', align: 'right', render: (v) => formatNumber(v, 2) },
    { title: '城市同比（%）', dataIndex: 'town_yoy', key: 'town_yoy', align: 'right', render: (v) => formatNumber(v, 2) },
    { title: '农村当月值', dataIndex: 'cnt_val', key: 'cnt_val', align: 'right', render: (v) => formatNumber(v, 2) },
    { title: '农村同比（%）', dataIndex: 'cnt_yoy', key: 'cnt_yoy', align: 'right', render: (v) => formatNumber(v, 2) },
  ];
