...
import { USTreasuryRealYieldCurve, getUSTreasuryRealYieldCurveList, USTreasuryRealYieldCurveQuery } from '@app/api/datamarket/macro.api';
import { useMacroData } from '../hooks/useMacroData';
import { useMacroSync } from '../hooks/useMacroSync';
import { formatNumber, getDateRanges } from '../utils';
import dayjs from 'dayjs';

export const USTreasuryRealYieldCurveTab: React.FC = () => {
  const { query, setQuery, rows, loading, pagination, total, fetchData } = useMacroData<USTreasuryRealYieldCurve, USTreasuryRealYieldCurveQuery>({
    fetchFn: async (params) => {
      const res = await getUSTreasuryRealYieldCurveList({
        skip: params.skip,
        limit: params.limit,
        date: params.date,
  ...
  const columns: ColumnsType<USTreasuryRealYieldCurve> = [
    { title: '日期', dataIndex: 'date', key: 'date', align: 'center', width: 120 },
    { title: '5年期实际利率（%）', dataIndex: 'y5', key: 'y5', align: 'right', render: (v) => formatNumber(v, 2) },
    { title: '7年期实际利率（%）', dataIndex: 'y7', key: 'y7', align: 'right', render: (v) => formatNumber(v, 2) },
    { title: '10年期实际利率（%）', dataIndex: 'y10', key: 'y10', align: 'right', render: (v) => formatNumber(v, 2) },
    { title: '20年期实际利率（%）', dataIndex: 'y20', key: 'y20', align: 'right', render: (v) => formatNumber(v, 2) },
    { title: '30年期实际利率（%）', dataIndex: 'y30', key: 'y30', align: 'right', render: (v) => formatNumber(v, 2) },
  ];
    ...
    <>
      ...
      <BaseSpace style={{ display: 'flex', marginBottom: '1rem' }}>
        <BaseButton
          type="primary"
          onClick={() => {
            openSync('us_treasury_real_yield_curve', {
