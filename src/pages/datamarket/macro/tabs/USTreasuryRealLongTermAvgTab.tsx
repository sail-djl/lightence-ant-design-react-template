...
import { USTreasuryRealLongTermAvg, getUSTreasuryRealLongTermAvgList, USTreasuryRealLongTermAvgQuery } from '@app/api/datamarket/macro.api';
import { useMacroData } from '../hooks/useMacroData';
import { useMacroSync } from '../hooks/useMacroSync';
import { formatNumber, getDateRanges } from '../utils';
import dayjs from 'dayjs';

export const USTreasuryRealLongTermAvgTab: React.FC = () => {
  const { query, setQuery, rows, loading, pagination, total, fetchData } = useMacroData<USTreasuryRealLongTermAvg, USTreasuryRealLongTermAvgQuery>({
    fetchFn: async (params) => {
      const res = await getUSTreasuryRealLongTermAvgList({
        skip: params.skip,
        limit: params.limit,
        date: params.date,
  ...
  const columns: ColumnsType<USTreasuryRealLongTermAvg> = [
    { title: '日期', dataIndex: 'date', key: 'date', align: 'center', width: 120 },
    { title: '实际平均利率（%）', dataIndex: 'ltr_avg', key: 'ltr_avg', align: 'right', render: (v) => formatNumber(v, 2) },
  ];
    ...
    <>
      ...
      <BaseSpace style={{ display: 'flex', marginBottom: '1rem' }}>
        <BaseButton
          type="primary"
          onClick={() => {
            openSync('us_treasury_real_long_term_avg', {
