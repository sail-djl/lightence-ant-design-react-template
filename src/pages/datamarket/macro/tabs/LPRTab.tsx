...
import { LPR, getLPRList, LPRQuery } from '@app/api/datamarket/macro.api';
import { useMacroData } from '../hooks/useMacroData';
import { useMacroSync } from '../hooks/useMacroSync';
import { formatNumber, getDateRanges } from '../utils';
import dayjs from 'dayjs';

export const LPRTab: React.FC = () => {
  const { query, setQuery, rows, loading, pagination, total, fetchData } = useMacroData<LPR, LPRQuery>({
    fetchFn: async (params) => {
      const res = await getLPRList({
        skip: params.skip,
        limit: params.limit,
        date: params.date,
  ...
  const columns: ColumnsType<LPR> = [
    { title: '日期', dataIndex: 'date', key: 'date', align: 'center', width: 120 },
    { title: '1年贷款利率（%）', dataIndex: 'rate_1y', key: 'rate_1y', align: 'right', render: (v) => formatNumber(v, 2) },
    { title: '5年贷款利率（%）', dataIndex: 'rate_5y', key: 'rate_5y', align: 'right', render: (v) => formatNumber(v, 2) },
  ];
