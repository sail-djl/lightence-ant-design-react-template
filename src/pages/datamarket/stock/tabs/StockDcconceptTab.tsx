...
import { StockDcconcept, getStockDcconceptList, syncStockDcconcept, StockDcconceptSyncPayload } from '@app/api/datamarket/stock.api';
...
// 东财概念及行业板块资金流向
export const StockDcconceptTab: React.FC = () => {
  ...
  >({
    fetchFn: async (params) => {
      const res = await getStockDcconceptList({
        skip: params.skip,
        limit: params.limit,
  ...
  const { syncOpen, setSyncOpen, syncLoading, syncPayload, setSyncPayload, handleSync } = useStockSync<
    StockDcconceptSyncPayload
  >({
    syncFn: syncStockDcconcept,
    onSuccess: () => fetchData(pagination.current, pagination.pageSize),
  });

  const columns: ColumnsType<StockDcconcept> = [
    ...
    { title: '板块代码', dataIndex: 'code', key: 'code', align: 'center' },
    { title: '板块名称', dataIndex: 'name', key: 'name', align: 'left' },
    { title: '净流入(元)', dataIndex: 'net_amount', key: 'net_amount', align: 'right', render: (v: any) => {
      const num = toNumber(v);
      if (num === null) return '-';
      const color = num >= 0 ? '#f5222d' : '#52c41a';
      return <span style={{ color }}>{num >= 0 ? '+' : ''}{num.toLocaleString()}</span>;
    }},
    { title: '净流入占比(%)', dataIndex: 'net_amount_rate', key: 'net_amount_rate', align: 'right', render: (v: any) => formatNumber(v, 2) },
  ];
  ...
  return (
    <>
      ...
      <BaseModal title="东财概念及行业板块资金流向同步" open={syncOpen} onCancel={() => setSyncOpen(false)} confirmLoading={syncLoading} onOk={() => handleSync()}>
        <BaseForm layout="vertical">
          ...
          <BaseForm.Item label="资金类型（可选）">
            <BaseInput
              placeholder="行业、概念、地域"
              value={syncPayload.content_type}
              onChange={(e) => setSyncPayload({ ...syncPayload, content_type: trim(e.target.value) || undefined })}
            />
          </BaseForm.Item>
        </BaseForm>
      </BaseModal>
