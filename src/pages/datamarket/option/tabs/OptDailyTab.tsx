...
import { OptDaily, getOptDailyList, OptDailyQuery } from '@app/api/datamarket/option.api';
...
export const OptDailyTab: React.FC = () => {
  const { query, setQuery, rows, loading, pagination, total, fetchData } = useOptionData<OptDaily, OptDailyQuery>({
    fetchFn: async (params) => {
      const res = await getOptDailyList({
        skip: params.skip,
        limit: params.limit,
  ...
  const columns: ColumnsType<OptDaily> = [
    ...
    <>
      ...
      >
        <BaseForm layout="vertical">
          <BaseForm.Item label="TS代码">
            <BaseInput
              placeholder="如：M1707-C-2400.DCE（不输入为获取全部）"
