...
export const ConfigFormModal: React.FC<ConfigFormModalProps> = ({
  ...
  // 根据分类和类型生成配置键
  const generateConfigKey = (category: string, type: string): string => {
    if (!category || !type) {
      return 'default';
    }
  ...
  // 判断配置键是否是自动生成的格式（category-type）
  const isAutoGeneratedConfigKey = (key: string, category: string, type: string): boolean => {
    if (!category || !type) {
      return false;
    }
    const expectedKey = generateConfigKey(category, type);
  ...
  const handleConfigTypeChange = (value: unknown) => {
    ...
    // 当类型改变时，如果分类已选择，自动生成配置键
    if (configCategory && type) {
      const autoKey = generateConfigKey(configCategory, type);
      const currentKey = form.getFieldValue('config_key');
      ...
      const shouldAutoFill = 
        ...
        (oldType && isAutoGeneratedConfigKey(currentKey, configCategory, oldType)); // 配置键匹配旧的分类-类型组合
        ...
        <S.ConfigFormArea>
          <BaseForm form={form} layout="vertical" onValuesChange={handleFormValuesChange}>
            <S.FormCard>
              ...
              >
                <BaseInput
                  placeholder={configCategory && configType ? generateConfigKey(configCategory, configType) : 'default'}
                  onChange={(e) => {
                    const newKey = e.target.value;
                    // 如果用户手动修改了配置键，检查是否还是自动生成的格式
                    if (configCategory && configType) {
                      setIsAutoGeneratedKey(isAutoGeneratedConfigKey(newKey, configCategory, configType));
                    } else {
                      setIsAutoGeneratedKey(false);
                    }
                  }}
